    ...
    pi.on("tool_call", async (ev) => {
        ...
        if (needsDebug) {
            if (name === "intent") {
                ...
                return {
                    block: true,
                    message: "Blocked: 2 consecutive fails -> need intent{goal:'debug ...'} (got non-debug intent) before write/edit/bash",
                };
            }
            if (isMutating) {
                return {
                    block: true,
                    message: "Blocked: 2 consecutive fails -> need intent{goal:'debug ...'} before write/edit/bash",
                };
            }
        }
