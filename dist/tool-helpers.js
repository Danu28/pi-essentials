...
export function hasUnquotedSpacePath(check) {
    ...
    for (const seg of segments) {
        ...
        const withoutCmd = trimmed
            ...
            .replace(/^\s*-[\w-]+\s*/g, "")
            .replace(/^__Q__\s*/, "");
        if (!withoutCmd || withoutCmd === "__Q__")
            continue;
        // now withoutCmd should be file args; if it contains word space word slash => path with space
        if (/\w+\s+\w+[\\/]/.test(withoutCmd))
            return true;
        if (/\w+\s+\w+\.\w+/.test(withoutCmd) && /[\\/]/.test(withoutCmd))
            return true;
        ...
        if (/^[\w\-\s]+\.\w+$/.test(withoutCmd.trim()) && /\s/.test(withoutCmd.trim()))
            return true;
    }
    return false;
}
